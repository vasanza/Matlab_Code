...
%IdTx = [12 22]; %Transmitter 2
%IdTx = [23 33]; %Transmitter 6
%IdTx = [1 22]; %Transmitter 1 & 2
%IdTx = [12 33]; %Transmitter 2 & 6
...
path = fullfile('./data/2021_julio_T1_2_6_16Qu/');%data folder
...
for i=IdTx(1):IdTx(2)%go through folders
    ...
    data=load(fullfile(path1,filenames(IdDataType).name));%return a table file
    data=cell2mat(struct2cell(data));
    data(:,60)=[];%elimina la columna de los 4 cuadrantes
    data(:,1)=[];%elimina la columna 1 muestra
